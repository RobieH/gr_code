...
%   a(now) = 1).  
%
%   Requires a analyticW function (denoted analyticW below).  This function
...
Omegas = [Or, Om, Ov]; %vector of omegas for analyticW function
...
while deltaTf < Trange
    ...
    k1 = analyticW(Af(i-1), Omegas);
    k2 = analyticW(Af(i-1)+step/2*k1, Omegas);
    k3 = analyticW(Af(i-1)+step/2*k2, Omegas);
    k4 = analyticW(Af(i-1)+step*k3, Omegas);
...
while deltaTb < Trange
    ...
    k1 = analyticW(Ab(i-1), Omegas);
    k2 = analyticW(Ab(i-1)+step/2*k1, Omegas);
    k3 = analyticW(Ab(i-1)+step/2*k2, Omegas);
    k4 = analyticW(Ab(i-1)+step*k3, Omegas);
